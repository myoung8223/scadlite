...
- **Code Folding (Brace & Comment-Based):** The editor's fold gutter collapses `{ ... }` blocks, and a custom comment-keyword fold lets you wrap any section between `// REGION` and `// ENDREGION` marker lines to collapse it on demand. The markers are ordinary comments, so they remain fully cross-compatible with desktop OpenSCAD. This is especially handy for tucking away manually inlined library code (see the Compatibility Notes below).
...
- **Link Sharing & Link Workspace:** An optional Link Sharing mode encodes your current model into a shareable URL and copies it to your clipboard, so a model can be opened by anyone simply by following the link. Enabling it adds a second, independent **Link Workspace** alongside the Main Workspace—received or shared models live there, keeping them separate from your primary work—and you can toggle between the two workspaces freely. Both persist in `localStorage`. *(Best suited to smaller models or demos, since URLs are length-limited; it is not intended as a primary save method.)*
...
- [x] **Code Folding:** Collapse `{ ... }` brace blocks via the editor's fold gutter, plus custom `// REGION` / `// ENDREGION` comment-based folding for hiding sections such as inlined libraries.
...
- [ ] **Camera Movement Improvements:** Improve the camera movement, perhaps with translation acceleration.
...
- **Code Folding:** Use the fold arrows in the editor's gutter to collapse `{ ... }` blocks. You can also wrap any block of code between a `// REGION` line and a `// ENDREGION` line to make that whole section foldable—handy for collapsing long, manually inlined library code. Because the markers are plain comments, code using them still runs unchanged in desktop OpenSCAD.
...
- **Link Sharing & Workspaces:** Enable **Link Sharing** in Workspace Settings to unlock a second **Link Workspace** and a set of link controls in the toolbar. Use **Share Link** to encode your current model into a URL and copy it to your clipboard, **Update Link** to re-encode after edits, and **Switch Workspace** to toggle between the Main and Link workspaces. Opening a URL that contains an encoded model loads it into the Link Workspace (overwriting whatever was there). A few things to keep in mind: URLs are length-limited, so link sharing is best for smaller models or demos rather than as a save method; and sharing a link while the Main Workspace is selected will copy the Main Workspace's code into the Link Workspace. If you run multiple instances/tabs at once, note that both workspaces are shared, global storage rather than per-instance—so save important work to a `.scad` file rather than relying on the cache to hold multiple versions.
- **3D Viewport & Display:**
...
- **Workspace Settings (⚙️):** Click the Settings icon or press **[Ctrl] + [,]** to access Workspace Settings. Press **[F1]** to toggle the Help / OpenSCAD cheat-sheet overlay. Settings include:
  ...
  - **Toggle Link Sharing:** Enable or disable Link Sharing and the secondary Link Workspace.
...
- **External libraries:** `include <...>` / `use <...>` of external libraries are **not yet supported**, since the WASM build has no library filesystem to resolve them against. As a workaround, simpler libraries can be manually inlined at the bottom of a project and collapsed with the `// REGION` / `// ENDREGION` folding described above. Libraries that are more "flat"—with few internal interdependencies—are the best candidates for inlining (for example, MCAD). Larger, deeply interdependent libraries such as BOSL/BOSL2 would first need to be flattened, which is considerably more challenging.
